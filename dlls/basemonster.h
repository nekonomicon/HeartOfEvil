...
public:
	...
	float				m_NextCamCheckTime; // stupid HACK
...
// stuff written for new state machine
		...
		virtual int IRelationshipSound ( CSound *pSound );
		...
		virtual void CheckAttacks ( CBaseEntity *pTarget, float flDist );
		...
		virtual BOOL Evacuate ( Vector vecThreat, Vector vecViewOffset, float flMinDist, float flMaxDist );
		...
		virtual BOOL GetEnemy ( void );
		virtual void MakeDamageBloodDecal ( int cCount, float flNoise, TraceResult *ptr, const Vector &vecDir );
