...
void CSqueakGrenade :: Killed( entvars_t *pevAttacker, int iGib )
{
	...
	CSoundEnt::InsertSound ( bits_SOUND_COMBAT, Classify(), pev->origin, SMALL_EXPLOSION_VOLUME, 3.0 );
...
void CSqueakGrenade::HuntThink( void )
{
	...
	if ((m_flDie - gpGlobals->time <= 0.5) && (m_flDie - gpGlobals->time >= 0.3))
	{
		...
		CSoundEnt::InsertSound ( bits_SOUND_COMBAT, Classify(), pev->origin, 256, 0.25 );
	}
...
void CSqueakGrenade::SuperBounceTouch( CBaseEntity *pOther )
{
	...
	{
		// skittering sound
		CSoundEnt::InsertSound ( bits_SOUND_COMBAT, Classify(), pev->origin, 100, 0.1 );
	}
