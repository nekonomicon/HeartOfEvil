...
Schedule_t	slTurnRound[] =
{
	{
		tlTurnRound,
		ARRAYSIZE ( tlTurnRound ),
		bits_COND_HEAVY_DAMAGE	|
		bits_COND_HEAR_SOUND |
		bits_COND_PROVOKED,
		bits_SOUND_DANGER,
...
Task_t	tlSearchAndDestroy[] =
{
	...
	{ TASK_WAIT,				(float)1.0				},
	{ TASK_SET_SCHEDULE,		(float)SCHED_SEARCH_AND_DESTROY },
};

Schedule_t	slSearchAndDestroy[] =
{
	{
		tlSearchAndDestroy,
		ARRAYSIZE ( tlSearchAndDestroy ),
		bits_COND_NEW_ENEMY		|
...
Task_t	tlSuppress[] =
{
	...
	{ TASK_FACE_ENEMY,			(float)0					},
	{ TASK_RANGE_ATTACK1,		(float)0					},
	{ TASK_SET_SCHEDULE,		(float)SCHED_SURPRESSING_FIRE },
};

Schedule_t	slSurpress[] =
{
	{ 
		tlSuppress,
		ARRAYSIZE ( tlSuppress ), 
		bits_COND_NEW_ENEMY			|
...
Schedule_t* CTalkMonster :: GetScheduleOfType ( int Type )
{
	switch( Type )
	{
	case SCHED_TURN_ROUND:
		return slTurnRound;

	case SCHED_RETREAT:
		return slRetreat;

	case SCHED_SEARCH_AND_DESTROY:
		return slSearchAndDestroy;

	case SCHED_SURPRESSING_FIRE:
		return slSurpress;

	case SCHED_FOLLOW_CLOSE:
		return slFollowClose;
...
// Called by ClientCommand on recieving a 'talk' message

void TalkMonsterCommand( int iCommand )
{
	// Look for talkmonsters

	CBaseEntity * pTalkMonster = NULL;
	char *pszFriend;
		
	for (int i=0; i < TLK_CFRIENDS; i++)
	{
		pszFriend = CTalkMonster::m_szFriends[ i ];
		while (pTalkMonster = UTIL_FindEntityByClassname( pTalkMonster, pszFriend ))
		{
			if (((CTalkMonster *) pTalkMonster)->IsFollowing())
			{
				((CTalkMonster *) pTalkMonster)->PlayerCommand( iCommand );
			}
		}
	}
}
...
void CTalkMonster::PlayerCommand( int iCommand )
{
	...
	case TLK_COMMAND_COME_HERE:	
		ChangeSchedule( GetScheduleOfType( SCHED_FOLLOW_CLOSE ) ); 
		break;
	...
	case TLK_COMMAND_ATTACK:
		if (m_hEnemy == NULL || HasConditions( bits_COND_ENEMY_DEAD ) )
		{
			...
			{
				ChangeSchedule( GetScheduleOfType( SCHED_INVESTIGATE_SOUND ) );
			}
			else
			{
				ChangeSchedule( GetScheduleOfType( SCHED_SEARCH_AND_DESTROY ) );
			}
		}
	...
	case TLK_COMMAND_RETREAT:
		ChangeSchedule( GetScheduleOfType( SCHED_RETREAT ) );
		break;

	case TLK_COMMAND_SURPRESSING_FIRE:
		if ( HasConditions( bits_COND_SEE_ENEMY ) ) 
		{
			ChangeSchedule( GetScheduleOfType( SCHED_SURPRESSING_FIRE ) );
		}
		else
		{
			TraceResult tr;
			UTIL_TraceLine(pev->origin + pev->view_ofs, m_vecEnemyLKP, ignore_monsters, ignore_glass, ENT(pev)/*pentIgnore*/, &tr);
			if (tr.flFraction == 1.0) ChangeSchedule( GetScheduleOfType( SCHED_SURPRESSING_FIRE ) );
		}
		break;

	case TLK_COMMAND_OUTTA_MY_WAY:
		ChangeSchedule( GetScheduleOfType( SCHED_MOVE_AWAY_FOLLOW ) );
		break;

	case TLK_COMMAND_BEHIND_YOU:	
		ChangeSchedule( GetScheduleOfType( SCHED_TURN_ROUND ) ); 
		break;
	}
}
