...
void UTIL_SetFog( Vector col, int iFadeTime, int iStartDist, int iEndDist )
{
...
BOOL UTIL_PlayMP3( const char *szFileName, BOOL looped, float flStartTime )
{
	//ALERT( at_console, "Mp3: %s, Time: %f\n", szFileName, flStartTime );
	...
	if (CAmbientMP3::m_nMp3On == MP3_ON )
	{
		long ms = (long) (flStartTime * 1000.0);
...
void UTIL_WriteHoEConfig()
{
	if ( CEnvFog::m_nFogOn == FOG_UNKNOWN ) CEnvFog::m_nFogOn = CEnvFog::GetFog();
	if ( CAmbientMP3::m_nMp3On == MP3_UNKNOWN ) CAmbientMP3::m_nMp3On = CAmbientMP3::GetMP3();

	FILE *fp = NULL;
	char szFilename[MAX_PATH];

	GET_GAME_DIR( szFilename );
	strcat( szFilename, "/hoeconfig.cfg" );

	if (!(fp = fopen( szFilename, "w" ))) 
	{
		ALERT( at_console, "Couldn't open: %s for writing\n", szFilename );
		return;	
	}
...
BOOL UTIL_SearchHoEConfig( char * szParm, int ndefault )
{
	FILE *fp = NULL;
	char szFilename[MAX_PATH], szLine[260];
	int n, val;
	...
	if (!fp) // config.cfg not found
	{
		ALERT ( at_console, "Couldn't open: %s for reading\n", szFilename );
		return ndefault;
	}

	do
	{
		n = fscanf( fp, "%s", szLine );	// Get the next string of non-white space characters

		if ( !strcmp( szLine, szParm ) )	// If it's parameter we're after, find the value it should be set to
		{
			n = fscanf( fp, "%d", &val );
			break;
		}
	...
	if ( n == EOF )	// string not found in config.cfg
	{
		val = ndefault;
	}
...
void UTIL_GunshotDecalTrace( TraceResult *pTrace, int decalNumber )
{
	...
	Vector vecSmoke = pTrace->vecEndPos + pTrace->vecPlaneNormal * 10;
