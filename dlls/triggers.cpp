...
// ========================================
// Nam - TriggerMonsterAI
// ========================================

class CTriggerMonsterAI : public CBaseEntity
{
public:
	void Spawn( void );
	void Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value );
...
LINK_ENTITY_TO_CLASS( trigger_monsterai, CTriggerMonsterAI );


void CTriggerMonsterAI :: Spawn ( void )
{
...
void CTriggerMonsterAI::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	if (!ShouldToggle(useType, m_fState)) return;
...
#define TRIGGERMONSTERAI_MAX_MONSTERS 32

void CTriggerMonsterAI :: Think ( void )
{
	CBaseEntity *pList[TRIGGERMONSTERAI_MAX_MONSTERS];

	int count = UTIL_EntitiesInBox( pList, TRIGGERMONSTERAI_MAX_MONSTERS, pev->absmin, pev->absmax, (FL_MONSTER) );
	if ( count )
	{
		for ( int i = 0; i < count; i++ )
		{
			CBaseMonster *pMonster = NULL;
			pMonster = pList[i]->MyMonsterPointer();
			if ( !pMonster ) continue;

			if (	pMonster->m_MonsterState != MONSTERSTATE_NONE && 
					...
					FNullEnt( FIND_CLIENT_IN_PVS( pMonster->edict() ) )
				)
			{
				pMonster->Look( pMonster->m_flDistLook );
				pMonster->Listen();// check for audible sounds. 

				// now filter conditions.
				pMonster->ClearConditions( pMonster->IgnoreConditions() );
