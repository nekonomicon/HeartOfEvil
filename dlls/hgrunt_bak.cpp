...
#define HGRUNT_WEAPON_M16		0
#define HGRUNT_WEAPON_SHOTGUN	1
#define HGRUNT_WEAPON_M79		2
#define HGRUNT_WEAPON_NOGUN		3

#define HGRUNT_BODY_BOONIE		0
#define HGRUNT_BODY_HELMET		1
#define HGRUNT_BODY_HELMET2		2
#define HGRUNT_BODY_CAP			3
#define HGRUNT_BODY_ANGRY		4
...
//=========================================================
#define		HGRUNT_AE_BURST1		( 2 )
#define		HGRUNT_AE_BURST2		( 3 ) 
#define		HGRUNT_AE_BURST3		( 4 ) 
#define		HGRUNT_AE_RELOAD		( 5 )
#define		HGRUNT_AE_KICK			( 6 )
#define		HGRUNT_AE_GREN_TOSS		( 7 )
#define		HGRUNT_AE_GREN_LAUNCH	( 8 )
#define		HGRUNT_AE_GREN_DROP		( 9 )
#define		HGRUNT_AE_CAUGHT_ENEMY	( 10) // grunt established sight with an enemy (player only) that had previously eluded the squad.
#define		HGRUNT_AE_DROP_GUN		( 11) // grunt (probably dead) is dropping his mp5.
#define		HGRUNT_AE_SCRIPT_BURST1	( 12 )
#define		HGRUNT_AE_SCRIPT_BURST2	( 13 ) 
#define		HGRUNT_AE_SCRIPT_BURST3	( 14 ) 
#define		HGRUNT_AE_SCRIPT_GREN_LAUNCH	( 15 )
...
enum
{
	...
	SCHED_GRUNT_ESTABLISH_M79_LINE_OF_FIRE,// move to a location to set up an attack against the enemy. (usually when a friendly is in the way).
...
int HGRUNT_CLIP_SIZE[ NUM_WEAPONS ] = { 30, 7, 1 };

//=====================
// Spawn Flags
//=====================

#define SF_HGRUNT_HANDGRENADES	0x0064
...
public:
	...
	void Shoot ( BOOL scripted = FALSE );
	void Shotgun ( BOOL scripted = FALSE );
...
//	DEFINE_FIELD( CHGrunt, m_flLastEnemySightTime, FIELD_TIME ), // don't save, go to zero
	...
	DEFINE_FIELD( CHGrunt, m_fHandGrenades, FIELD_BOOLEAN ),
	DEFINE_FIELD( CHGrunt, m_CrouchTime, FIELD_TIME ),
	DEFINE_FIELD( CHGrunt, m_Crouching, FIELD_BOOLEAN ),
...
void CHGrunt :: GibMonster ( void )
{
	...
	if ( GetBodygroup( WEAPON_GROUP ) != HGRUNT_WEAPON_NOGUN )
	{// throw a gun if the grunt has one
		...
		switch ( pev->weapons )
		{
			case HGRUNT_WEAPON_M16:		pGun = DropItem( "weapon_m16", vecGunPos, vecGunAngles );		break;
			case HGRUNT_WEAPON_SHOTGUN:	pGun = DropItem( "weapon_870", vecGunPos, vecGunAngles );		break;
			case HGRUNT_WEAPON_M79:		pGun = DropItem( "weapon_m79", vecGunPos, vecGunAngles );		break;
		}
		if ( pGun )
		{
...
{
	if (pev->weapons == HGRUNT_WEAPON_M16 || pev->weapons == HGRUNT_WEAPON_SHOTGUN)
	{
...
{
	if (! m_fHandGrenades && GetBodygroup(WEAPON_GROUP) != HGRUNT_WEAPON_M79)
	{
		return FALSE;
	}
...
void CHGrunt :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	...
	if (ptr->iHitgroup == 11)
	{
		...
		if ((GetBodygroup(BODY_GROUP) == HGRUNT_BODY_HELMET || GetBodygroup(BODY_GROUP) == HGRUNT_BODY_HELMET2)
...
//		CTalkMonster::TakeDamage(pevInflictor, pevAttacker, flDamage, bitsDamageType);
		...
		if (InSquad())
		{
			...
			for (int i = 0; i < MAX_SQUAD_MEMBERS; i++)
			{
				...
				if (pSquadMember) pSquadMember->Remember( bits_MEMORY_PROVOKED );
			}
		}
	}
...
// Shoot
//=========================================================
void CHGrunt :: Shoot ( BOOL scripted )
{
	if (m_hEnemy == NULL)
	{
		return;
	...
	if (scripted) vecShootDir = gpGlobals->v_forward;
...
// Shoot
//=========================================================
void CHGrunt :: Shotgun ( BOOL scripted )
{
	if (m_hEnemy == NULL)
	{
		return;
...
void CHGrunt :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case HGRUNT_AE_DROP_GUN:
		...
		{// throw a gun if the grunt has one
			...
			switch ( pev->weapons )
			{
				case HGRUNT_WEAPON_M16:		DropItem( "weapon_m16", vecGunPos, vecGunAngles );		break;
				case HGRUNT_WEAPON_SHOTGUN:	DropItem( "weapon_870", vecGunPos, vecGunAngles );		break;
				case HGRUNT_WEAPON_M79:		DropItem( "weapon_m79", vecGunPos, vecGunAngles );		break;
			}
			SetBodygroup( WEAPON_GROUP, HGRUNT_WEAPON_NOGUN );
		}
		break;
	...
	case HGRUNT_AE_SCRIPT_GREN_LAUNCH:
		UTIL_MakeVectors(pev->angles);
		m_vecTossVelocity = gpGlobals->v_forward * 800;

	case HGRUNT_AE_GREN_LAUNCH:
	{
		if (RANDOM_LONG(0, 1))
			EMIT_SOUND(ENT(pev), CHAN_WEAPON, "weapons/m79_fire1.wav", 0.8, ATTN_NORM);
		else
			EMIT_SOUND(ENT(pev), CHAN_WEAPON, "weapons/m79_fire2.wav", 0.8, ATTN_NORM);
	...
	case HGRUNT_AE_SCRIPT_BURST1:
	{
		switch( pev->weapons )
		{
		case HGRUNT_WEAPON_M16:
			Shoot( pEvent->event == HGRUNT_AE_SCRIPT_BURST1 );
			EMIT_SOUND_DYN( ENT(pev), CHAN_WEAPON, "weapons/m16_burst.wav", 1, ATTN_NORM, 0, 100 );
			break;

		case HGRUNT_WEAPON_SHOTGUN:
			Shotgun( pEvent->event == HGRUNT_AE_SCRIPT_BURST1 );
	...
	case HGRUNT_AE_SCRIPT_BURST2:
		Shoot( pEvent->event == HGRUNT_AE_SCRIPT_BURST3 );
		break;

	case HGRUNT_AE_BURST3:
	case HGRUNT_AE_SCRIPT_BURST3:
		if (pev->weapons == HGRUNT_WEAPON_M16) 
			Shoot( pEvent->event == HGRUNT_AE_SCRIPT_BURST3 );
		else
...
void CHGrunt :: Spawn()
{
	...
	m_iSentence			= HGRUNT_SENT_NONE;
	m_cClipSize			= HGRUNT_CLIP_SIZE[ pev->weapons ];
	...
	if ( pev->body > NUM_BODIES ) pev->body = -1;
	if ( pev->body == -1 )
	...
	if (GetBodygroup( WEAPON_GROUP ) != HGRUNT_WEAPON_M79 )
	{
		m_fHandGrenades = pev->spawnflags & SF_HGRUNT_HANDGRENADES;
	}
	else
	{
		m_fHandGrenades = FALSE;
	}
...
void CHGrunt :: StartTask ( Task_t *pTask )
{
	...
	case TASK_GET_M79_PATH_TO_ENEMY:
		if ( m_hEnemy == NULL )
		{
			TaskFail();
			return;
		...
		if ( BuildNearestRoute( m_hEnemy->pev->origin, m_hEnemy->pev->view_ofs, 256, 1024 ) )
		{
			TaskComplete();
		}
		else
...
//=========================================================
// M79 Establish line of fire - move to a position that allows
// the grunt to attack without killing himself.
//=========================================================
Task_t tlGruntEstablishM79LineOfFire[] = 
{
	{ TASK_SET_FAIL_SCHEDULE,	(float)SCHED_GRUNT_ESTABLISH_LINE_OF_FIRE	},  // if fail, try and go up to enemy and kick him inna nuts
	{ TASK_GET_M79_PATH_TO_ENEMY,(float)0						},
	{ TASK_GRUNT_SPEAK_SENTENCE,(float)0						},
	{ TASK_RUN_PATH,			(float)0						},
	{ TASK_WAIT_FOR_MOVEMENT,	(float)0						},
};

Schedule_t slGruntEstablishM79LineOfFire[] =
{
	{ 
		tlGruntEstablishM79LineOfFire,
		ARRAYSIZE ( tlGruntEstablishM79LineOfFire ),
		bits_COND_NEW_ENEMY			|
		bits_COND_ENEMY_DEAD		|
		bits_COND_CAN_RANGE_ATTACK1	|
		bits_COND_CAN_MELEE_ATTACK1	|
...
//	{ TASK_PLAY_SEQUENCE_FACE_ENEMY,	(float)/*ACT_SIGNAL2*/ 0		},
	{ TASK_FACE_ENEMY,					(float)0				},
...
{
	{ TASK_STOP_MOVING,			(float)0		},
//	{ TASK_PLAY_SEQUENCE_FACE_ENEMY,		(float)/*ACT_CROUCH*/ 0 },
	{ TASK_FACE_ENEMY,			(float)0		},
	{ TASK_GRUNT_CHECK_FIRE,	(float)0		},
	{ TASK_RANGE_ATTACK1,		(float)0		},
	{ TASK_FACE_ENEMY,			(float)0		},
...
{
	{ TASK_STOP_MOVING,				(float)0		},
//	{ TASK_PLAY_SEQUENCE_FACE_ENEMY,(float)/*ACT_IDLE_ANGRY*/0  },
	{ TASK_FACE_ENEMY,			(float)0		},
	{ TASK_GRUNT_CHECK_FIRE,	(float)0		},
	{ TASK_RANGE_ATTACK1,		(float)0		},
	{ TASK_FACE_ENEMY,			(float)0		},
...
void CHGrunt :: SetActivity ( Activity NewActivity )
{
	...
	case ACT_MELEE_ATTACK1:
		strcpy( seq, "frontkick" );
		m_Crouching = FALSE;
		pev->view_ofs = Vector ( 0, 0, 68 );
		m_CrouchTime = gpGlobals->time + 15.0;
		break;
	...
	if ( HasConditions(bits_COND_HEAR_SOUND) )
	{
		...
		if ( pSound)
		{
			...
			if (pSound->m_iType & (bits_SOUND_DANGER | bits_SOUND_COMBAT | bits_SOUND_PLAYER))
				if (m_CrouchTime < gpGlobals->time) Crouch();
			...
			if ( HasConditions(bits_COND_NEW_ENEMY) )
			{
				if ( InSquad() )
				{
					...
					else 
					{
						...
						{
							if (pev->weapons == HGRUNT_WEAPON_M79) 
								return GetScheduleOfType ( SCHED_GRUNT_ESTABLISH_M79_LINE_OF_FIRE );
							else
...
// can grenade launch
			...
			else if ( (GetBodygroup(GUN_GROUP) == HGRUNT_WEAPON_M79) 
				&& HasConditions ( bits_COND_CAN_RANGE_ATTACK2 ) && OccupySlot( bits_SLOTS_HUMAN_GRENADE ) )
			{
...
Schedule_t* CHGrunt :: GetScheduleOfType ( int Type ) 
{
	...
	case SCHED_TAKE_COVER_FROM_ENEMY:
		{
			if ( InSquad() )
			{
				if ( m_fHandGrenades && g_iSkillLevel == SKILL_HARD && HasConditions( bits_COND_CAN_RANGE_ATTACK2 ) && OccupySlot( bits_SLOTS_HUMAN_GRENADE ) )
				{
					if (FOkToSpeak())
					{
						SENTENCEG_PlayRndSz( ENT(pev), "HG_THROW", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
			...
			{
				if ( m_fHandGrenades && RANDOM_LONG(0,1) )
				{
	...
	case SCHED_GRUNT_ESTABLISH_LINE_OF_FIRE:
		...
		break;
	case SCHED_GRUNT_ESTABLISH_M79_LINE_OF_FIRE:
		{
			return &slGruntEstablishM79LineOfFire[ 0 ];
		}
		break;
...
int CHGrunt::CheckEnemy(CBaseEntity *pEnemy)
{
...
void CHGruntRepel::RepelUse ( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	*/
	...
	if (pev->spawnflags & SF_HGRUNT_FRIENDLY) pGrunt->pev->spawnflags &= SF_HGRUNT_FRIENDLY;
...
char *CDeadHGrunt::m_szPoses[] = { "lying_on_back", "lying_on_side", "seated", "lying_on_stomach" };
...
void CDeadHGrunt :: Spawn( void )
{
	...
	SetBodygroup( BODY_GROUP, pev->body );
	SetBodygroup( GUN_GROUP, HGRUNT_WEAPON_NOGUN );
