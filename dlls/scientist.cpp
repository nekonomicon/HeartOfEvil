...
#define		NUM_BODIES		9 // 9 head body combinations available for scientist (peasant) model of either sex

int MALE_BODY[NUM_BODIES] =  { 0, 1, 2, 
				               6, 7, 8,
				              12,13,14 };		//Valid male bodies

int FEMALE_BODY[NUM_BODIES] = { 21, 22, 23,
					            27, 28, 29,
				                33, 34, 35 };	//Valid female bodies
												//(so we don't get female heads on male bodies or vice versa)
...
#define		SCIENTIST_AE_DROPGRENADE ( 4 )
...
//LINK_ENTITY_TO_CLASS( monster_peasant, CScientist );
...
void CScientist::DeclineFollowing( void )
{
	...
	if ( pev->body < NUM_BODIES * 2 )
		PlaySentence( "PEASANT_MALE", 2, VOL_NORM, ATTN_NORM );
	else
		PlaySentence( "PEASANT_FEMALE", 2, VOL_NORM, ATTN_NORM );
}
...
void CScientist :: Scream( void )
{
	if ( FOkToSpeak() )
	{
		Talk( 10 );
		m_hTalkTarget = m_hEnemy;
		PlaySentence( "PEASANT_SCREAM", RANDOM_FLOAT(3, 6), VOL_NORM, ATTN_NORM );
	}
}
...
//			else
				PlaySentence( "PEASANT_FEAR", 5, VOL_NORM, ATTN_NORM );
		}
		TaskComplete();
		break;
...
//	m_flDistTooFar		= 256.0;
	...
	if ( pev->body == -1 )
	{// -1 chooses whether to be male or female
		pev->body = 0 - (2 + RANDOM_LONG(0, 1));
	}

	if ( pev->body == -2 )
	{// -2 chooses a random male head-body combination
		pev->body = MALE_BODY[ RANDOM_LONG(0, (NUM_BODIES)-1) ];// pick a head-body combination
	}

	if ( pev->body == -3 )
	{// -3 chooses a random female head-body combination
		pev->body = FEMALE_BODY[ RANDOM_LONG(0, (NUM_BODIES)-1) ];// pick a head-body combination
	}
...
void CScientist :: TalkInit()
{
	...
	m_szFriends[3] = "monster_mikeforce";

	// scientists speach group names (group names are in sentences.txt)

	// get sentences for gender
	if ( pev->body < NUM_BODIES * 2 )
		m_szGrp[TLK_PIDLE] = "PEASANT_MALE";
	else
		m_szGrp[TLK_PIDLE] = "PEASANT_FEMALE";
}
