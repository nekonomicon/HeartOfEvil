...
BOOL CSquadMonster :: OccupySlot( int iDesiredSlots )
{
	...
	if ( m_iMySlot != bits_NO_SLOT )
	{
		//ALERT( at_console, "OccupySlot called on a monster who already had one\n" );
		VacateSlot();
	}
...
void CSquadMonster :: VacateSlot()
{
	...
	{
		ALERT ( at_aiconsole, "Vacated Slot %d - %d\n", m_iMySlot, MySquadLeader()->m_afSquadSlots );
		...
		while ((pEntity = UTIL_FindEntityInSphere( pEntity, pev->origin, searchRadius )) != NULL)
		{
			...
			if ( pRecruit && pRecruit != this && pRecruit->IsAlive() && !pRecruit->m_pCine )
			{
				...
				if ( !pRecruit->InSquad() && pRecruit->Classify() == iMyClass &&
				   ( (iMyClass != CLASS_ALIEN_PREY) || FStrEq(STRING(pev->classname), STRING(pRecruit->pev->classname))) &&
...
void CSquadMonster :: StartMonster( void )
{
	...
	if ( ( m_afCapability & bits_CAP_SQUAD ) && !InSquad() )
	{
		...
		if ( iSquadSize )
		{
		  ALERT ( at_console, "Squad of %d %s formed\n", iSquadSize, STRING( pev->classname ) );
		}
...
/*	if ( m_hEnemy != NULL )
	...
	UTIL_MakeVectors ( UTIL_VecToAngles( m_vecEnemyLKP - pev->origin ) );
