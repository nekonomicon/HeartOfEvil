...
void CEnvSound :: Think( void )
{
	...
	if (FEnvSoundInRange(pev, VARS(pentPlayer), &flRange)) 
	{
		if (flRange < pPlayer->m_flSndRange || pPlayer->m_flSndRange == 0) 
		{
			...
			ALERT( at_console, "env_sound type: %f\n", m_flRoomtype );
...
#define CSENTENCEG_MAX 300					// max number of sentence groups
// globals
...
char TEXTURETYPE_Find(char *name)
{
	...
	for (int i = 0; i < gcTextures; i++)
	{
		...
		{
			//ALERT( at_console, "Texture: %s Type: %c\n", grgszTextureName[i], grgchTextureType[i] );
			return (grgchTextureType[i]);
		}
	}

	//ALERT( at_console, "Texture: %s Type: concrete\n", name );
...
// hit the world, try to play sound based on texture material type
	...
	if ((iBulletType == BULLET_PLAYER_MACHETE) &&
		(chTextureType == CHAR_TEX_CONCRETE ||
		chTextureType == CHAR_TEX_METAL ||
		chTextureType == CHAR_TEX_VENT ||
		chTextureType == CHAR_TEX_GRATE ||
		chTextureType == CHAR_TEX_TILE))
	{
		rgsz[0] = "weapons/machete_metal1.wav";
		rgsz[1] = "weapons/machete_metal2.wav";
		rgsz[2] = "weapons/machete_metal3.wav";
		cnt = 3;	
		fvol = 0.9;
		fvolbar = 0.0;	
	}
	else switch (chTextureType)
	{
	default:
	...
	case CHAR_TEX_COMPUTER:
		...
		break;
	case CHAR_TEX_FLESH:
		if (iBulletType == BULLET_PLAYER_CROWBAR  || iBulletType == BULLET_PLAYER_MACHETE || 
			iBulletType == BULLET_PLAYER_CHAINSAW || iBulletType == BULLET_PLAYER_PISTOLWHIP )
		...
		cnt = 2;
		break;
	case CHAR_TEX_MUD: fvol = 0.9; fvolbar = 0.3;
		rgsz[0] = "player/pl_mud1.wav";
		rgsz[1] = "player/pl_mud2.wav";
		rgsz[2] = "player/pl_mud3.wav";
		rgsz[3] = "player/pl_mud4.wav";
		cnt = 4;
		break;
	}
...
{
	DEFINE_FIELD( CSpeaker, m_flAttenuation, FIELD_FLOAT ),
	DEFINE_FIELD( CSpeaker, m_preset, FIELD_INTEGER ),
	DEFINE_FIELD( CSpeaker, m_mindelay, FIELD_FLOAT ),
	DEFINE_FIELD( CSpeaker, m_maxdelay, FIELD_FLOAT ),
};
...
void CSpeaker :: Precache( void )
{
	if ( !FBitSet (pev->spawnflags, AMBIENT_SOUND_START_SILENT ) )
		...
		pev->nextthink = gpGlobals->time + RANDOM_FLOAT(m_mindelay, m_maxdelay);
...
void CSpeaker :: SpeakerThink( void )
{
	...
	{
		pev->nextthink = gpGlobals->time + RANDOM_FLOAT(m_mindelay, m_maxdelay);
		ALERT( at_console, "Speaker waiting for humans to stop talking\n" );
		return;
	}
	...
	if (m_preset)
	{
		...
		switch (m_preset)
		{
		case 1: szSoundFile =  "NEAR_WAR"; break;
		case 2: szSoundFile =  "FAR_WAR"; break;
		case 3: szSoundFile =  "JUNGLE"; break;
		}
	} else
	...
	if (szSoundFile[0] == '!')
	{
		...
		UTIL_EmitAmbientSound ( ENT(pev), pev->origin, szSoundFile, 
			flvolume, m_flAttenuation, flags, pitch);

		// shut off and reset
		pev->nextthink = 0.0;
		ALERT( at_console, "Speaker played sentence and shut off\n" );
	}
	else
	{
		// make random announcement from sentence group

		if (SENTENCEG_PlayRndSz(ENT(pev), szSoundFile, flvolume, m_flAttenuation, flags, pitch) < 0)
			ALERT(at_console, "Level Design Error!\nSPEAKER has bad sentence group name: %s\n",szSoundFile); 

		// set next announcement time for random delay
		pev->nextthink = gpGlobals->time + RANDOM_FLOAT(m_mindelay, m_maxdelay);

		CHuman::g_talkWaitTime = gpGlobals->time + 5;		// time delay until it's ok to speak: used so that two NPCs don't talk at once
		ALERT( at_console, "Speaker played random sentence\n" );
	}
