...
#if 1
	//if ( FClassnameIs( pev, "monster_mikeforce" ) || FClassnameIs( pev, "monster_mikeforce_medic" ))
	{
	...
	case TASK_MOVE_TO_TARGET_RANGE:
		{
			...
			else
			{
				...
				else if ( distance >= 270 && m_movementActivity != ACT_RUN && LookupActivity( ACT_RUN ) != ACTIVITY_NOT_AVAILABLE )
	...
	case TASK_SET_SCHEDULE:
		{
			...
			if ( pNewSchedule )
			{
				ScheduleChange();	// Inform monster that his schedule is about to change
				ChangeSchedule( pNewSchedule );
			}
			else
			{
...
//				return;
	...
	case TASK_FIND_COVER_FROM_BEST_SOUND:
		{
			...
			*/
			...
			else if ( pBestSound && Evacuate( pBestSound->m_vecOrigin, g_vecZero, pBestSound->m_iVolume, CoverRadius() ) )
			{
				// then try for plain ole cover
				m_flMoveWaitFinished = gpGlobals->time + pTask->flData;
				TaskComplete();
	...
	case TASK_EAT:	// Eat - set me to not hungry for a while AND increase my health
		{
			Eat( pTask->flData );
			TakeHealth( pev->health + pTask->flData, DMG_GENERIC );
			TaskComplete();
			break;
		}
	...
	case TASK_SOUND_WAKE:
		{
			if (pTask->flData == 0 || RANDOM_FLOAT(0,1) <= pTask->flData) AlertSound();
			TaskComplete();
			break;
		}
	case TASK_SOUND_DIE:
		{
			if (pTask->flData == 0 || RANDOM_FLOAT(0,1) <= pTask->flData) DeathSound();
			TaskComplete();
			break;
		}
	case TASK_SOUND_IDLE:
		{
			if (pTask->flData == 0 || RANDOM_FLOAT(0,1) <= pTask->flData) IdleSound();
			TaskComplete();
			break;
		}
	case TASK_SOUND_PAIN:
		{
			if (pTask->flData == 0 || RANDOM_FLOAT(0,1) <= pTask->flData) PainSound();
			TaskComplete();
			break;
		}
	...
	case TASK_SOUND_ANGRY:
		{
			...
			if (pTask->flData == 0 || RANDOM_FLOAT(0,1) <= pTask->flData) AttackSound();
			TaskComplete();
			break;
		}
	case TASK_SOUND_EAT:
