...
{
	if ( gpGlobals->time - m_flLastHurtTime < 5 && 
		( pTarget->Classify() == CLASS_ALIEN_PREY || pTarget->Classify() == CLASS_ALIEN_PASSIVE ) )
	{
		// if squid has been hurt in the last 5 seconds, and is getting relationship for a headcrab or chumtoad, 
		// tell squid to disregard crab or toad. 
		return R_NO;
	}
...
{
	if ( m_hEnemy != NULL && m_hEnemy->pev->groundentity && Instance(m_hEnemy->pev->groundentity) == this ) return FALSE;
...
{
	if ( m_hEnemy != NULL && m_hEnemy->pev->groundentity && Instance(m_hEnemy->pev->groundentity) == this ) return TRUE;
		...
		case BSQUID_AE_THROW:
			{
				...
				else // If someone is on top of me throw them off
				{
					TraceResult tr;
					Vector vecStart = pev->origin + gpGlobals->v_forward * 16;
					vecStart.z += pev->size.z * 0.5;
					Vector vecEnd = vecStart + (gpGlobals->v_up * 70 );
					...
					if ( tr.pHit )
					{
						...
						if ( pEntity && pEntity->pev->groundentity && Instance(pEntity->pev->groundentity) == this )
						{
							pEntity->pev->velocity = pEntity->pev->velocity - gpGlobals->v_forward * 300 + gpGlobals->v_up * 300;
						}
					}
				}
			}
...
Task_t tlSquidWallow[] =
{
	...
	{ TASK_DONT_EAT,				(float)50				},// keeps squid from eating or sniffing anything else for a while.
	{ TASK_GET_PATH_TO_LASTPOSITION,(float)0				},
	{ TASK_WALK_PATH,				(float)0				},
	{ TASK_WAIT_FOR_MOVEMENT,		(float)0				},
	{ TASK_CLEAR_LASTPOSITION,		(float)0				},
