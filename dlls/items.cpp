...
class CItemHelmet : public CItem
{
	...
	BOOL MyTouch( CBasePlayer *pPlayer )
	{
			...
			(pPlayer->pev->weapons & (1<<WEAPON_SUIT)))
		{
			...
			pPlayer->pev->armorvalue += gSkillData.helmet;
...
LINK_ENTITY_TO_CLASS(item_battery, CItemHelmet);
...
class CItemFlakJacket : public CItem
{
	...
	{ 
		Precache( );
		SET_MODEL(ENT(pev), "models/w_flakjacket.mdl");
		CItem::Spawn( );
	}
	void Precache( void )
	{
		PRECACHE_MODEL ("models/w_flakjacket.mdl");
		PRECACHE_SOUND( "items/gunpickup2.wav" );
	}
	BOOL MyTouch( CBasePlayer *pPlayer )
	{
			...
			(pPlayer->pev->weapons & (1<<WEAPON_SUIT)))
		{
			...
			pPlayer->pev->armorvalue += gSkillData.flakjacket;
			pPlayer->pev->armorvalue = min(pPlayer->pev->armorvalue, MAX_NORMAL_BATTERY);
