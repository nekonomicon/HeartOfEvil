...
void ClientCommand( edict_t *pEntity )
{
	...
	else if ( FStrEq(pcmd, "talk_come_here" ) )
	{
		CHumanFollower::PlayerSquadCommand( SQUADCMD_COME_HERE );
	}
	else if ( FStrEq(pcmd, "talk_attack" ) )
	{
		CHumanFollower::PlayerSquadCommand( SQUADCMD_ATTACK );
	}
	else if ( FStrEq(pcmd, "talk_retreat" ) )
	{
		CHumanFollower::PlayerSquadCommand( SQUADCMD_RETREAT );
	}
	else if ( FStrEq(pcmd, "talk_get_down" ) )
	{
		CHumanFollower::PlayerSquadCommand( SQUADCMD_GET_DOWN );
	}
	else if ( FStrEq(pcmd, "talk_surpressing_fire" ) )
	{
		CHumanFollower::PlayerSquadCommand( SQUADCMD_SURPRESSING_FIRE );
	}
	else if ( FStrEq(pcmd, "talk_outta_my_way" ) )
	{
		CHumanFollower::PlayerSquadCommand( SQUADCMD_OUTTA_MY_WAY );
	}
	else if ( FStrEq(pcmd, "talk_behind_you" ) )
	{
		CHumanFollower::PlayerSquadCommand( SQUADCMD_BEHIND_YOU );
	}
	else if ( FStrEq(pcmd, "playmp3" ) )
	{
		UTIL_PlayMP3( (char *)CMD_ARGV(1), FALSE, 0 );
	}
	else if ( FStrEq(pcmd, "mp3" ) )
	{
		const char *pArgv1 = CMD_ARGV(1);
		
		if ( FStrEq(pArgv1, "0" ) )
		{
			CAmbientMP3::m_nMp3On = MP3_OFF;
			CAmbientMP3::SetMP3();
			CLIENT_COMMAND ( pEntity, "stopmp3\n");
		}
		else if ( FStrEq(pArgv1, "1" ) )
		{
			CAmbientMP3::m_nMp3On = MP3_ON;
			CAmbientMP3::SetMP3();
		}
		
		ClientPrint( &pEntity->v, HUD_PRINTCONSOLE, UTIL_VarArgs( "mp3 is set to: %d\n", CAmbientMP3::m_nMp3On ) );
	}
	else if ( FStrEq(pcmd, "fog" ) )
	{
		...
		if ( FStrEq(pArgv1, "0" ) )
		{
			CEnvFog::m_nFogOn = FOG_OFF;
			CEnvFog::SetFog();
			UTIL_Fog( CEnvFog::m_nFogOn );
		}
		else if ( FStrEq(pArgv1, "1" ) )
		{
			CEnvFog::m_nFogOn = FOG_ON;
			CEnvFog::SetFog();
			UTIL_Fog( CEnvFog::m_nFogOn );
		}
		
		ClientPrint( &pEntity->v, HUD_PRINTCONSOLE, UTIL_VarArgs( "fog is set to: %d\n", CEnvFog::m_nFogOn ) );
	}
	else
	{
...
void ClientPrecache( void )
{
	...
	PRECACHE_SOUND("player/pl_mud1.wav");		// walk on mud
