...
void CGib :: SpawnStickyGibs( entvars_t *pevVictim, Vector vecOrigin, int cGibs )
{
	...
	for ( i = 0 ; i < cGibs ; i++ )
	{
		...
		if ( pevVictim )
		{
			...
			pGib->pev->avelocity.x = RANDOM_FLOAT ( 300, 400 );
			pGib->pev->avelocity.y = RANDOM_FLOAT ( 300, 400 );
...
void CGib :: SpawnRandomGibs( entvars_t *pevVictim, int cGibs, int human )
{
	...
	for ( cSplat = 0 ; cSplat < cGibs ; cSplat++ )
	{
		...
		else
		{
			...
			case GIB_PALM:
				{
					pGib->Spawn( "models/palmgibs.mdl" );
					pGib->pev->body = RANDOM_LONG(0,PALM_GIB_COUNT-1);
				}
				break;

			case GIB_HUMAN_STICKY:
				{
					pGib->Spawn( "models/stickygib.mdl" );
					pGib->pev->body = RANDOM_LONG(0,STICKY_GIB_COUNT-1);
					pGib->SetTouch ( StickyGibTouch );
				}
				break;

			case GIB_ALIEN_STICKY:
				{
					pGib->Spawn( "models/agibs.mdl" );
					pGib->pev->body = RANDOM_LONG(0,ALIEN_GIB_COUNT-1);
					pGib->SetTouch ( StickyGibTouch );
			...
			case GIB_HGRUNT:
				{
					// Grunts
					pGib->Spawn( "models/namgruntgibs.mdl" );
					pGib->pev->body = cSplat;
				}
				break;
			...
			case GIB_MIKEFORCE:
				{
					// mikes
					pGib->Spawn( "models/mikeforcegibs.mdl" );
					pGib->pev->body = cSplat;
				}
				break;
			...
			case GIB_SOG:
				{
					// SOGs
					pGib->Spawn( "models/soggibs.mdl" );
					pGib->pev->body = cSplat;
				}
				break;

			case GIB_HGRUNTMEDIC:
				{
					// Grunt Medics
					pGib->Spawn( "models/gruntmedicgibs.mdl" );
					pGib->pev->body = cSplat;
				}
				break;
			...
			case GIB_ARMYMEDIC:
				{
					// Army Medics
					pGib->Spawn( "models/armymedicgibs.mdl" );
					pGib->pev->body = cSplat;
				}
				break;

			case GIB_CHARLIE_NVA_BROWN:
				{
					pGib->Spawn( "models/charliegibs.mdl" );
					pGib->pev->skin = 1;
					pGib->pev->body = cSplat;
				}
				break;
			
			case GIB_CHARLIE_NVA_GREEN:
				{
					pGib->Spawn( "models/charliegibs.mdl" );
					pGib->pev->skin = 0;
					pGib->pev->body = cSplat;
		...
		if ( pevVictim )
		{
			...
			if ( human == GIB_ALIEN_STICKY || human == GIB_HUMAN_STICKY )
			{
				pGib->pev->velocity = pGib->pev->velocity * 2;
			}
			if ( pevVictim->health > -50)
...
void CBaseMonster :: GibMonster( void )
{
	...
	if ( HasHumanGibs() )
	{
		...
		{
			CGib::SpawnHeadGib( pev );	// erm, HACKHACK
			CGib::SpawnRandomGibs( pev, 4, GIB_HUMAN );	// throw some human gibs.
	...
	else if ( HasAlienGibs() )
	{
		...
		{
			CGib::SpawnRandomGibs( pev, 4, GIB_ALIEN );	// Throw alien gibs
...
void CGib :: WaitTillLand ( void )
{
	...
	if ( pev->velocity == g_vecZero )
	{
		...
		{
			// ok, start stinkin!
			CSoundEnt::InsertSound ( bits_SOUND_MEAT, Classify(), pev->origin, 384, 25 );
		}
	}
	else
	{
...
//
// Sticky gib - same as normal gibs but with BIG decals
//
...
BOOL CBaseEntity :: FVisible ( CBaseEntity *pEntity )
{
	...
	// Return false if target is, uh, invisible
	if ( pEntity->pev->rendermode == kRenderTransTexture && pEntity->pev->renderamt == 0 )
		return FALSE;
...
void CBaseMonster :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	if ( pev->takedamage )
	{
		...
		if ( FBitSet( bitsDamageType, DMG_BULLET ) )	// For bullet damage, do a stream of blood out the other direction
		{
			switch ( BloodColor() )
			{
			case BLOOD_COLOR_RED: UTIL_BloodStream( ptr->vecEndPos, vecDir, 70, 50 + flDamage * 5 ); break;
			case BLOOD_COLOR_GREEN: UTIL_BloodStream( ptr->vecEndPos, vecDir, BloodColor(), 50 + flDamage * 5 ); break;
			}
		}
	}
}
...
void CBaseEntity::FireBullets(ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker )
{
	...
	for (ULONG iShot = 1; iShot <= cShots; iShot++)
	{
		...
		// Make tracer sound if close to player's ear

		if (iShot < 4 && !IsPlayer() && (!pEntity || !(pEntity->IsPlayer())))
		{
			...
			if (pPlayer)
			{
				Vector v = vecEnd - vecSrc;
				Vector w = pPlayer->EarPosition() - vecSrc;

				double c1 = DotProduct(w,v);
				double c2 = DotProduct(v,v);
				double b = c1 / c2;

				Vector Pb = vecSrc + b * v;

//				ALERT(at_console, "Now: %f\n", (Pb - pPlayer->EarPosition()).Length());
				if (b >= 0.0f && b <= 1.0f && (Pb - pPlayer->EarPosition()).Length() < 128.0f)
				{

					switch (RANDOM_LONG(0, 9))
					{
					case 0:	UTIL_EmitAmbientSound(ENT(0), Pb, "weapons/tracer1.wav", 1.0f, ATTN_NORM, 0, 100); break;
					case 1:	UTIL_EmitAmbientSound(ENT(0), Pb, "weapons/tracer2.wav", 1.0f, ATTN_NORM, 0, 100); break;
					case 2:	UTIL_EmitAmbientSound(ENT(0), Pb, "weapons/tracer3.wav", 1.0f, ATTN_NORM, 0, 100); break;
					case 3:	UTIL_EmitAmbientSound(ENT(0), Pb, "weapons/tracer4.wav", 1.0f, ATTN_NORM, 0, 100); break;
					case 4:	UTIL_EmitAmbientSound(ENT(0), Pb, "weapons/tracer5.wav", 1.0f, ATTN_NORM, 0, 100); break;
					case 5:	UTIL_EmitAmbientSound(ENT(0), Pb, "weapons/tracer6.wav", 1.0f, ATTN_NORM, 0, 100); break;
					case 6:	UTIL_EmitAmbientSound(ENT(0), Pb, "weapons/tracer7.wav", 1.0f, ATTN_NORM, 0, 100); break;
					case 7:	UTIL_EmitAmbientSound(ENT(0), Pb, "weapons/tracer8.wav", 1.0f, ATTN_NORM, 0, 100); break;
					case 8:	UTIL_EmitAmbientSound(ENT(0), Pb, "weapons/tracer9.wav", 1.0f, ATTN_NORM, 0, 100); break;
					case 9:	UTIL_EmitAmbientSound(ENT(0), Pb, "weapons/tracer10.wav",1.0f, ATTN_NORM, 0, 100); break;
					}
				}
			}
		}
		...
		//draw a puff of smoke where it hits whatever it hits
/*		if ( iBulletType == BULLET_MONSTER_12MM ) 
		{
			CSprite *pSprite = CSprite::SpriteCreate( "sprites/steam1.spr", tr.vecEndPos + Vector(0, 0, 5), TRUE );
			pSprite->SetTransparency( kRenderTransAlpha, 50, 40, 0, 150, kRenderFxNone );
			pSprite->SetScale( 1 );
//			pSprite->AnimateAndDie( 10 );
			pSprite->SetThink(pSprite->AnimateUntilDead); 
			pSprite->pev->framerate = 10;
			pSprite->pev->dmgtime = gpGlobals->time + 0.8; 
...
*/
		...
		if (tr.flFraction != 1.0)
		{
			...
			case BULLET_PLAYER_M21:		
				pEntity->TraceAttack(pevAttacker, gSkillData.plrDmgM21, vecDir, &tr, DMG_BULLET); 
				break;

			case BULLET_PLAYER_1143:		
				pEntity->TraceAttack(pevAttacker, gSkillData.plrDmg1143, vecDir, &tr, DMG_BULLET); 
				break;
			...
			case BULLET_MONSTER_SHOT:
				pEntity->TraceAttack(pevAttacker, gSkillData.monDmgShot, vecDir, &tr, DMG_BULLET);
				break;

			case BULLET_MONSTER_AK47:
				pEntity->TraceAttack(pevAttacker, gSkillData.monDmgAK47, vecDir, &tr, DMG_BULLET);
				break;
			...
			case BULLET_MONSTER_1143:
				pEntity->TraceAttack(pevAttacker, gSkillData.monDmg1143, vecDir, &tr, DMG_BULLET);
				break;
			...
			if (iShot < 8) TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
