...
#define SF_MONSTERMAKER_FADE_CHILDREN 16	// Children fade
...
TYPEDESCRIPTION	CMonsterMaker::m_SaveData[] = 
{
	...
	DEFINE_FIELD( CMonsterMaker, m_iszChildTarget, FIELD_STRING ),
...
void CMonsterMaker :: KeyValue( KeyValueData *pkvd )
{
	...
	else if ( FStrEq(pkvd->szKeyName, "childtarget") )
	{
		m_iszChildTarget = ALLOC_STRING( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
...
void CMonsterMaker :: Spawn( )
{
	...
	if ( FBitSet ( pev->spawnflags, SF_MONSTERMAKER_FADE_CHILDREN ) )
	{
...
void CMonsterMaker::MakeMonster( void )
{
	...
	CBaseEntity * pBaseEntity = Instance( pent );
	pBaseEntity->ApplyDefaultSettings();

	CBaseMonster * pMonster = pBaseEntity->MyMonsterPointer();
	if (pMonster)
	{
	...
	// Child target
	if ( !FStringNull ( m_iszChildTarget ) )
	{
		pevCreate->target = m_iszChildTarget;
	}
