//
// Implementation of the CHumanMedic class
//
//=========================================================
...
void CHumanMedic :: Spawn( void )
{
	CHuman::Spawn();
}


void CHumanMedic :: Precache( void )
{
	CHuman::Precache();
}
...
void CHumanMedic :: SetActivity ( Activity NewActivity )
{
	...
	switch ( NewActivity)
	{
		...
		default:
			CHuman::SetActivity( NewActivity );
			return;
			break;
	}
...
void CHumanMedic :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	switch( pEvent->event )
	{
	case HUMAN_MEDIC_AE_SYRINGE_HELMET:
		SetBodygroup( GetSyringeGroupNum(), HUMAN_MEDIC_SYRINGE_HELMET );
		break;

	case HUMAN_MEDIC_AE_SYRINGE_HAND:
		SetBodygroup( GetSyringeGroupNum(), HUMAN_MEDIC_SYRINGE_HAND );
		break;

	case HUMAN_MEDIC_AE_HEAL:
		Heal();
		break;
...
//=========================================================
// Heal target, provided he is close enough - and not dead
//=========================================================

void CHumanMedic::Heal( void )
{
	if ( m_hTargetEnt == NULL || !m_hTargetEnt->IsAlive() || m_hTargetEnt->pev->deadflag == DEAD_DYING ||
		m_hTargetEnt->pev->health <= 0 ) return;

	Vector target = m_hTargetEnt->pev->origin - pev->origin;
	if ( target.Length() > 100 )
		return;

	m_hTargetEnt->TakeHealth( gSkillData.medicHeal, DMG_GENERIC );
}
...
void CHumanMedic :: StartTask( Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	...
	case TASK_HUMAN_MEDIC_CHECK_TARGET:
		{
			...
			if ( m_hTargetEnt!=NULL && m_hTargetEnt->pev->deadflag == DEAD_NO )
			{
				...
				CHuman * pHuman = m_hTargetEnt->MyHumanPointer();
...
Schedule_t *CHumanMedic :: GetSchedule ( void )
{
	...
	case MONSTERSTATE_COMBAT:
		{
			...
			if ( HasConditions( bits_COND_ENEMY_OCCLUDED ) )
			{
				if ( IsFollowingHuman() )
				{
					...
					{
						m_hTargetEnt->MyHumanPointer()->StopFollowing( TRUE );	// we know m_hTargetEnt is CHuman from above
						StopFollowing( TRUE );
						return GetSchedule();
					}
					else
					{
						return GetScheduleOfType( SCHED_HUMAN_MEDIC_CHASE );
					}
				}
...
void CHumanMedic::Killed( entvars_t *pevAttacker, int iGib )
{
	if ( m_nHeadNum == GetNumHeads() ) SetBodygroup( GetSyringeGroupNum(), HUMAN_MEDIC_SYRINGE_NONE );
...
Task_t	tlHumanMedicHeal[] =
{
	{ TASK_HUMAN_MEDIC_CHECK_TARGET,	(float)0					},	// Check if he's still alive, no point healing a dead man
	{ TASK_HUMAN_MEDIC_SOUND_HEAL,		(float)0					},  // Tell target to hold still, this won't hurt a bit
	{ TASK_PLAY_SEQUENCE_FACE_TARGET,	(float)ACT_SPECIAL_ATTACK2	},	// Whip out the needle
	{ TASK_SET_ACTIVITY,				(float)ACT_IDLE				},
};

Schedule_t	slHumanMedicHeal[] =
{
	{
		tlHumanMedicHeal,
		ARRAYSIZE ( tlHumanMedicHeal ),
...
Task_t	tlHumanMedicChase[] =
{
	{ TASK_MOVE_TO_TARGET_RANGE,	(float)64						},	// Move within 64 of target ent (client)
	{ TASK_FACE_IDEAL,				(float)0						},
	{ TASK_SET_SCHEDULE,			(float)SCHED_HUMAN_MEDIC_HEAL	},
};

Schedule_t	slHumanMedicChase[] =
{
	{
		tlHumanMedicChase,
		ARRAYSIZE ( tlHumanMedicChase ),
...
DEFINE_CUSTOM_SCHEDULES( CHumanMedic )
{
	slHumanMedicHeal,
	slHumanMedicChase,
};

IMPLEMENT_CUSTOM_SCHEDULES( CHumanMedic, CHuman );
...
Schedule_t* CHumanMedic :: GetScheduleOfType ( int Type )
{
	switch( Type )
	{
		case SCHED_HUMAN_MEDIC_HEAL:
		{
			return &slHumanMedicHeal[ 0 ];
		}
		break;

		case SCHED_HUMAN_MEDIC_CHASE:
		{
			return &slHumanMedicChase[ 0 ];
		}
		break;
	}
